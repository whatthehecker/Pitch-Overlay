...
impl MyApp {
    fn new(input_devices: Vec<Device>) -> Self {
        Self {
                                ...
                                if ui.selectable_value(&mut self.current_device_index, Some(i), name).clicked() {
                                    ...
                                    self.current_stream = Some(self.available_input_devices[i].build_input_stream(
                                        ...
                                        move |data: &[i16], info| {
                                            ...
                                            let mut audio_state = cloned_arc.lock().unwrap();
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let response = plot.show(ui, move |plot_ui| {
                ...
                plot_ui.set_plot_bounds(PlotBounds::from_min_max(
                    [0.0, self.min_display_frequency as f64],
                    [10.0, self.max_display_frequency as f64],
                ));
                plot_ui.line(Line::new(PlotPoints::new(cloned_arc.lock().unwrap().pitch_points.clone())));
